...
moveit::planning_interface::PlanningSceneInterface* ur5_planning_scene_interface;
...
std::string SceneBase::PrintVector(const geometry_msgs::Pose &v, int precis){
  ...
  ss << v.orientation.x << " " << v.orientation.y << " " << v.orientation.z << " " << v.orientation.w << ")";
  return ss.str();
}

bool SceneBase::ApplyAttachedCollisionObject(moveit_msgs::AttachedCollisionObject &msg, double delay){
  if (delay < 0){
    delay = CALL_DILAY;
  }
  try{
...
bool SceneBase::ApplyCollisionObject(moveit_msgs::CollisionObject &msg, double delay){
  if (delay < 0){
    delay = CALL_DILAY;
  }
  try{
...
SceneBase::SceneBase(ros::NodeHandle &node, const std::string PLANNING_GROUP)
{
  ...
  if (PLANNING_GROUP.compare("") != 0){
    ur5_move_group_interface = new moveit::planning_interface::MoveGroupInterface(PLANNING_GROUP);
    ur5_joint_model_group = ur5_move_group_interface->getCurrentState()->getJointModelGroup(PLANNING_GROUP);  // Pointer for speed
  }

  ur5_planning_scene_interface = new moveit::planning_interface::PlanningSceneInterface; //&ur5_psi;
  ...
  ur5_planning_scene = new planning_scene::PlanningScene(ur5_KinematicModel); // &ur5_PlanningScene;
}
