#include "../include/arm_control/Planner.hpp"

bool Planner::SvcCallback_PlanPath(arm_control::PlanPath::Request &req,
  ...
  if (req.ClearAllPlans) {
    ...
    if (PathPlans.size() > 0) {
      ...
      start_state = new robot_state::RobotState(
          *ur5_move_group_interface->getCurrentState());
    ...
    if (req.AddAsJointSpace) {
      ...
      ROS_INFO_STREAM("IK was " << (pose_valid ? "" : "NOT ")
                                << "solved successfully.");
      if (pose_valid) {
    ...
    if (pose_valid) {
      ...
      plan_success = (ur5_move_group_interface->plan(js_plan) ==
                      moveit::planning_interface::MoveItErrorCode::SUCCESS);
    } else {
      plan_success = false;
    }

    ROS_INFO_STREAM("Plan from "
                    << std::endl
                    << "  " << PrintVector(joint_pos0) << " to " << std::endl
                    << "  " << PrintVector(joint_pos)
                    << (plan_success ? " " : " NOT ") << "successful.");
    if (plan_success) {
    ...
    if (req.AddWaypointOnly) {
      ...
      arm_control::ServiceInt GripperColl;
      /*
      GripperColl.request.Data = -1;
      svc_EnableGripper.call(GripperColl);
      */

      // ROS_INFO_STREAM((GripperEnabled ? "Gripper is enabled..." : "Gripper is
      // NOT enabled..."));
      ...
      if (GripperEnabled && req.ExcludeGripper) {
        ...
        while (!svc_EnableGripper.call(GripperColl)) {
          ROS_WARN(
              "Failed to call gripper collision service to disable collision.");
        }
      ...
      double fraction = ur5_move_group_interface->computeCartesianPath(
          Waypoints, 0.01, req.Speed, trajectory);
      plan_success = fraction > 0.999;

      std::stringstream ss;
      ss << "Path from " << std::endl
         << "  Base point: " << PrintVector(pose0) << " to" << std::endl;
      ...
      ROS_INFO_STREAM(ss.str()
                      << " was " << fraction * 100 << "% feasible. Plan "
                      << (plan_success ? "" : "NOT ") << "successful.");
      ...
      if (GripperEnabled && req.ExcludeGripper) {
        ...
        while (!svc_EnableGripper.call(GripperColl)) {
          ROS_WARN(
              "Failed to call gripper collision service to enable collision.");
        }
        /*
        ur5_control::ServiceInt::Request greq;
        ur5_control::ServiceInt::Response grep;
...
bool Planner::SvcCallback_PlanExecute(arm_control::ServiceInt::Request &req,
...
bool Planner::SubClientGripper() {
  bool flag = svc_EnableGripper.waitForExistence(ros::Duration(1.0));
  if (!flag) {
    for (int i = 0; i < 3; i++) {
      flag = svc_EnableGripper.waitForExistence(ros::Duration(1.0));
      if (flag) {
        svc_EnableGripper = RosNode->serviceClient<arm_control::ServiceInt>(
            "ur5_control/scene/collision/gripper_coll_enabled", 1);
        break;
      }
    }
    if (!flag) {
...
void Planner::SubCallback_Wall(
  ...
  /*BaseSence 的函数，目前是添加或是删除wall_obj 而调用 */
...
Planner::Planner(ros::NodeHandle &node, const std::string &PLANNING_GROUP)
  ...
  for (auto i : keys) {
    ...
    if (idx1 != std::string::npos) {
      ...
      if (idx2 != std::string::npos) {
        StaticTFBroadcaster->sendTransform(
            paramFrameToMsg(node, i.substr(16, idx2 - 16)));
      }
    }
  }
  ...
  */
  ...
  srv_PlanPath = RosNode->advertiseService(
      "ur5_control/plan/queue", &Planner::SvcCallback_PlanPath, this);
  srv_PlanExecute = RosNode->advertiseService(
      "ur5_control/plan/execute", &Planner::SvcCallback_PlanExecute, this);
  svc_EnableGripper = RosNode->serviceClient<arm_control::ServiceInt>(
      "ur5_control/scene/collision/gripper_coll_enabled");
  sub_EnableWall = RosNode->subscribe<std_msgs::Float64MultiArray>(
      "ur5_control/scene/collision/wall", 100, &Planner::SubCallback_Wall,
      this);

  // Wait 5 seconds for gripper collision service to come up (requires mobile
  // base TF)
