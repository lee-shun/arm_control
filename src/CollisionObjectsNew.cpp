#include "../include/arm_control/CollisionObjectsNew.hpp"

bool CollHandler::SvcCallback_CollCheck(arm_control::CollCheck::Request &req,
                                        arm_control::CollCheck::Response &res) {

  sensor_msgs::JointState joint;
  collision_detection::CollisionRequest collision_request,
      collision_request_self;
  collision_detection::CollisionResult collision_result, collision_result_self;
 ...
 */
bool CollHandler::SvcCallback_CollisionGripperEnabled(
...
bool CollHandler::AddBaseObjects() {
...
bool CollHandler::AddGripperObjects(std::string baseframe) {
...
bool CollHandler::AddCollisionGripper(bool add, double delay) {
  if (add) {
...
void CollHandler::SubCallback_process_coll_msg(
...
CollHandler::CollHandler(ros::NodeHandle &node,
                         const std::string &PLANNING_GROUP, bool loadbase,
                         bool loadgripper)
  ...
  /**
   * 如果载入基座或者抓手，那么设定碰撞检测以及注册碰撞检测的服务
   */
  ...
  /**
   * matlab->"apple_new"->AppleTrack->"process_msg",实际上就添加了苹果的位置
   */
  ...
  try {
    ...
    T_W_B_msg =
        TFBuffer->lookupTransform("world", "mobile_base", ros::Time(0), tfwait);
